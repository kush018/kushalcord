...
public class WorkCommand implements Command {
    ...
    public void execute(MessageCreateEvent event, String[] argv, String argvStr) {
        ...
        if (Main.userWorkCooldownTimerMap.get(event.getMessage().getAuthor().get()) == null
                || Main.userWorkCooldownTimerMap.get(event.getMessage().getAuthor().get()).get() == 0) {
            //if cooldown is over, then we set the cooldown to how much time is specified in USER_WORK_TIME_OUT
            //and continue as normal
            Main.userWorkCooldownTimerMap.put(event.getMessage().getAuthor().get(), new AtomicLong(USER_WORK_TIME_OUT));
        } else {
        ...
        try {
            ...
            Main.currentWorkingJobsList.add(job);
        } catch (ArrayIndexOutOfBoundsException e) {
            //if an argument was not mentioned for the job, then it will throw an ArrayIndexOutOfBoundsException
            Main.userWorkCooldownTimerMap.put(event.getMessage().getAuthor().get(), new AtomicLong(0));
            event.getMessage().getChannel().block().createEmbed((embed) -> {
                embed.setColor(Color.DISCORD_WHITE);
